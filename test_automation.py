...
DEFAULT_EXCEL_CANDIDATES = [
    "/Users/sahanvaas/Documents/Y3S1/ITPM/Assignment_01/test_automation/Assignment 1 - Test cases.xlsx",
...
def _ensure_input_value(page, input_locator, text: str, type_delay_ms: int):
    ...
    # Use fill for stability (especially long Singlish text)
    try:
        input_locator.fill(text)
    except Exception as e:
        print("Input fill failed:", e)
        return
    ...
    try:
        ...
        if current and str(current).strip() == text.strip():
            return
    except Exception:
        pass
...
def _find_chat_locators(page, timeout_ms: int):
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if count >= 2:
                candidates = [textareas.nth(i) for i in range(count)]
                visible = [c for c in candidates if c.is_visible()]

                if len(visible) >= 2:
                    input_box = visible[0]
                    output_box = visible[1]

                    action = page.locator("button:has-text('Transliterate')").first

                    return input_box, output_box, action
        except Exception:
            pass
    ...
    raise RuntimeError("Could not locate input/output textareas on the page")
...
def run_test():
    ...
    # Fixed column mapping based on provided Excel headers
    input_col_idx = _find_column_index(header_values, "Input", DEFAULT_INPUT_COLUMN_CANDIDATES)
    expected_col_idx = _find_column_index(header_values, "Expected output", DEFAULT_EXPECTED_COLUMN_CANDIDATES)

    # ALWAYS ensure output columns exist
    actual_col_idx = _ensure_column(ws, header_row, header_values, "Actual Output")
    status_col_idx = _ensure_column(ws, header_row, header_values, "Status")
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                # 🔥 CLEAN INPUT (fixed reliable React-safe version)
                try:
                ...
                # 🔥 click translate button (Transliterate) with retry + "Failed to fetch" handling
                actual_output = ""

                for attempt in range(max(1, int(args.retries))):
                    try:
                        translate_btn = page.locator("button:has-text('Transliterate')").first
                        translate_btn.wait_for(state="visible", timeout=5000)
                        translate_btn.click(timeout=5000)
                    except Exception as e:
                        print(f"Translate click failed (attempt {attempt+1}): {e}")

                    page.wait_for_timeout(1000)

                    # check for fetch failure message
                    page_text = ""
                    try:
                    ...
                    if "Failed to fetch" in page_text:
                        print(f"Detected 'Failed to fetch' (attempt {attempt+1}) -> retrying...")
                        page.wait_for_timeout(max(500, int(args.retry_wait_ms)))
                        continue

                    break

                # 🚀 Robust wait for slow UI (poll until output updates or timeout)
                actual_output = ""
                start_time = time.time()
                timeout_sec = max(5, int(args.timeout_ms) / 1000)

                while time.time() - start_time < timeout_sec:
                    try:
                        current_output = _read_output(is_chat, output_locator)
                    except Exception:
                        current_output = ""

                    if current_output and current_output != prev_output:
                        actual_output = current_output
                        break

                    page.wait_for_timeout(300)

                # final fallback read (ensures we capture last value)
